        ...
        <div className="flex flex-col">
                    ...
                    <div key={step} className={`flex-1 rounded-sm ${bgClass}`}>
                       {isCurrent && (
                          <div className="absolute -top-6 left-1/2 -translate-x-1/2 flex flex-col items-center">
                             <span className="text-xs font-bold text-gray-900 dark:text-white tabular-nums">{data.currentRating}</span>
          ...
          <div className="mt-auto bg-gray-50 dark:bg-white/[0.02] rounded-lg p-3 border border-black/[0.05] dark:border-white/[0.05]">
             ...
             <div className="flex justify-between text-xs text-gray-600 dark:text-gray-400">
                <span>Avg Δ (last 10): <strong className="text-gray-900 dark:text-white">{data.avgDeltaLast10 > 0 ? `+${data.avgDeltaLast10}` : data.avgDeltaLast10}</strong></span>
                <span>Est. contests: <strong className="text-gray-900 dark:text-white">{data.estimatedContests}</strong></span>
             </div>
          </div>
        </div>
