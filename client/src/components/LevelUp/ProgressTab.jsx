...
function ChartTooltip({ active, payload, label }) {
  ...
  return <div className="bg-[#1a1a1a] border border-[#333] rounded-xl px-3 py-2 shadow-xl text-xs"><p className="text-gray-300 font-medium mb-1">{label}</p>{payload.map((p,i)=><div key={i} className="flex items-center gap-2"><span className="w-2 h-2 rounded-full" style={{backgroundColor:p.color||p.fill}}/><span className="text-gray-400 capitalize">{p.dataKey}</span><span className="ml-auto font-bold text-white font-[family-name:'JetBrains_Mono']">{p.value}</span></div>)}</div>;
}
...
export default function ProgressTab() {
  ...
  if (loading) return <motion.div initial={{opacity:0}} animate={{opacity:1}} className="space-y-3">{[...Array(4)].map((_,i)=><div key={i} className="bg-[#111] border border-[#222] rounded-xl animate-pulse" style={{height:i===0?80:160}}/>)}</motion.div>;

  if (error) return (
    <motion.div initial={{opacity:0,y:10}} animate={{opacity:1,y:0}} className="bg-[#121212]/50 border border-gray-800/60 rounded-2xl p-8 backdrop-blur-xl text-center py-16">
      <AlertCircle className="w-12 h-12 text-red-400 mx-auto mb-4"/><p className="text-gray-400 mb-1 font-medium">Failed to load performance stats</p>
  ...
  if (!data || (data.overview.solved === 0 && data.overview.submissions === 0)) return (
    <motion.div initial={{opacity:0,y:10}} animate={{opacity:1,y:0}} className="bg-[#121212]/50 border border-gray-800/60 rounded-2xl p-8 backdrop-blur-xl">
      <div className="flex items-center gap-4 mb-6"><div className="p-3 bg-emerald-500/10 rounded-xl border border-emerald-500/20"><Activity className="text-emerald-500 w-5 h-5"/></div><div><h2 className="text-xl font-bold text-white">Performance Stats</h2><p className="text-sm text-gray-400">Your coding journey over the last 30 days</p></div></div>
      <div className="text-center py-12 border-2 border-dashed border-gray-800 rounded-xl"><BarChart3 className="w-10 h-10 text-gray-600 mx-auto mb-3"/><p className="text-gray-500 font-medium">No submissions in the last 30 days</p></div>
    </motion.div>
  );
  ...
  const cm = {
    emerald:{t:'text-emerald-500',bg:'bg-emerald-500/10',hb:'hover:border-emerald-500/30'},
    blue:{t:'text-blue-500',bg:'bg-blue-500/10',hb:'hover:border-blue-500/30'},
    violet:{t:'text-violet-500',bg:'bg-violet-500/10',hb:'hover:border-violet-500/30'},
    amber:{t:'text-amber-500',bg:'bg-amber-500/10',hb:'hover:border-amber-500/30'},
    orange:{t:'text-orange-500',bg:'bg-orange-500/10',hb:'hover:border-orange-500/30'},
    cyan:{t:'text-cyan-500',bg:'bg-cyan-500/10',hb:'hover:border-cyan-500/30'},
  };
  ...
  return (
    <motion.div initial={{opacity:0,y:10}} animate={{opacity:1,y:0}} transition={{duration:0.4}} className="space-y-3">
      ...
      {/* ── Row 0: Header + Info ─── */}
      <div className="bg-[#121212]/50 border border-gray-800/60 rounded-2xl px-5 py-4 backdrop-blur-xl">
        <div className="flex items-center gap-3 mb-2">
          ...
          <div><h2 className="text-lg font-bold text-white tracking-tight">Performance Stats</h2><p className="text-xs text-gray-400">Last 30 days vs previous month</p></div>
        </div>
        <div className="flex items-start gap-2 p-2.5 bg-blue-500/5 border border-blue-500/15 rounded-lg">
          <Info className="w-3.5 h-3.5 text-blue-400 mt-0.5 flex-shrink-0"/>
          <p className="text-[11px] text-blue-300/80 leading-relaxed">Re-solving old problems counts as separate submissions — revisiting builds pattern recognition and contest speed.</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-3">
          ...
          <Card delay={0.05} className="lg:col-span-3">
            ...
            <div className="space-y-1.5">
              {insights.slice(0,3).map((ins,i) => {
                ...
                return <div key={i} className={`flex items-start gap-2.5 p-2.5 rounded-lg border text-xs ${INSIGHT_STYLES[ins.type]||INSIGHT_STYLES.info}`}><Ic className="w-3.5 h-3.5 mt-0.5 flex-shrink-0"/><p className="leading-relaxed">{ins.text}</p></div>;
              })}
            </div>
          </Card>
        )}
        <Card delay={0.08} className="flex flex-col items-center justify-center">
          ...
          <p className="text-[10px] uppercase tracking-wider text-gray-500 mt-1">Consistency</p>
          <p className="text-2xl font-bold font-[family-name:'JetBrains_Mono'] text-white">{overview.consistencyScore||0}<span className="text-sm text-gray-500">/100</span></p>
        </Card>
      </div>
        ...
        {statCards.map((s,i) => {
          ...
          return (
            <motion.div key={s.l} initial={{opacity:0,y:10}} animate={{opacity:1,y:0}} transition={{delay:0.03*i}}
              className={`bg-[#0f0f0f] border border-[#1a1a1a] rounded-xl p-3 ${c.hb} transition-all`}>
              <div className="flex items-center gap-1.5 mb-1.5"><div className={`p-1 rounded-md ${c.bg}`}><Ic className={`w-3 h-3 ${c.t}`}/></div><span className="text-[9px] font-medium uppercase tracking-wider text-gray-500">{s.l}</span></div>
              <div className="text-xl font-bold font-[family-name:'JetBrains_Mono'] text-white leading-none">{s.v}</div>
              {comp && <DeltaChip current={comp.current} previous={comp.previous}/>}
              {s.extra && <span className="inline-flex items-center gap-1 text-[9px] font-semibold mt-1 px-1.5 py-0.5 rounded-full text-amber-400 bg-amber-500/10"><Trophy className="w-2.5 h-2.5"/>{s.extra}</span>}
            </motion.div>
          );
        })}
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-3">
        <Card delay={0.15} className="lg:col-span-2">
          <div className="flex items-center justify-between mb-3">
            ...
            <span className="text-[10px] text-gray-500 font-[family-name:'JetBrains_Mono']">{overview.activeDays}/30 active</span>
          </div>
          <div className="flex gap-[4px] flex-wrap">
            {last30Days.map(day => {
              const cnt = activityMap[day]||0;
              const int = cnt===0?'bg-[#1a1a1a]':cnt<=1?'bg-emerald-900/40':cnt<=3?'bg-emerald-700/60':cnt<=6?'bg-emerald-600/80':'bg-emerald-500';
          ...
          <div className="flex items-center gap-1.5 mt-2.5 text-[9px] text-gray-500">
            <span>Less</span>
            {['bg-[#1a1a1a]','bg-emerald-900/40','bg-emerald-700/60','bg-emerald-600/80','bg-emerald-500'].map((c,i)=><div key={i} className={`w-3 h-3 rounded ${c}`}/>)}
            <span>More</span>
          </div>
        </Card>
            ...
            <div className="flex items-end gap-1.5 justify-between h-24">
              {dayOfWeek.map((d,i) => {
                ...
                return <div key={d.day} className="flex-1 flex flex-col items-center gap-1">
                  <span className={`text-[9px] font-bold font-[family-name:'JetBrains_Mono'] ${mx?'text-emerald-400':'text-gray-600'}`}>{d.count}</span>
                  ...
                  <span className={`text-[9px] ${mx?'text-emerald-400':'text-gray-600'}`}>{d.day}</span>
                </div>;
              })}
            </div>
          </Card>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-3">
        {weaknessTags?.length > 0 && (
          <Card delay={0.2}>
            <Title icon={ShieldAlert} color="text-red-400">Weak Areas</Title>
            <div className="space-y-2">
              {weaknessTags.slice(0,4).map((t,i) => (
                <div key={t.tag} className="flex items-center gap-2">
                  <span className="text-[11px] text-gray-300 w-24 truncate">{t.tag}</span>
                  ...
                  <span className={`text-[10px] font-bold font-[family-name:'JetBrains_Mono'] w-8 text-right ${t.rate<30?'text-red-400':t.rate<60?'text-amber-400':'text-emerald-400'}`}>{t.rate}%</span>
                </div>
              ))}
            </div>
          </Card>
            ...
            <div className="space-y-2">
              {strengthTags.slice(0,4).map((t,i) => (
                <div key={t.tag} className="flex items-center gap-2">
                  ...
                  <span className="text-[10px] font-bold font-[family-name:'JetBrains_Mono'] w-8 text-right text-emerald-400">{t.rate}%</span>
                </div>
              ))}
            </div>
          </Card>
        ...
        {(difficultyProgression?.current > 0 || difficultyProgression?.hardest > 0) && (
          <Card delay={0.24}>
            ...
            <div className="grid grid-cols-3 gap-2">
              {[{l:'Avg Now',v:difficultyProgression.current,c:'text-white'},{l:'Avg Prev',v:difficultyProgression.previous||'—',c:'text-gray-400'},{l:'Hardest',v:difficultyProgression.hardest,c:'text-emerald-400'}].map(d=>(
                <div key={d.l} className="text-center p-2 bg-[#0c0c0c] rounded-lg border border-[#1a1a1a]">
                  <p className="text-[9px] uppercase tracking-wider text-gray-500 mb-0.5">{d.l}</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-3">
        <Card delay={0.26}>
          ...
          ) : <div className="h-[160px] flex items-center justify-center text-gray-600 text-sm">No data</div>}
          <div className="flex items-center gap-3 mt-2 justify-center">
            {Object.entries(PLATFORM_META).map(([k,m])=><div key={k} className="flex items-center gap-1 text-[10px] text-gray-500"><span className="w-2 h-2 rounded-full" style={{backgroundColor:m.color}}/>{m.label}</div>)}
          </div>
        </Card>
            ...
            <div className="flex items-center gap-3">
              <div className="w-[120px] h-[120px] flex-shrink-0">
                <ResponsiveContainer width="100%" height="100%">
                  ...
                  </Pie><Tooltip contentStyle={{backgroundColor:'#1a1a1a',border:'1px solid #333',borderRadius:'10px',fontSize:'11px'}} itemStyle={{color:'#fff'}} formatter={(v,n)=>[`${v} (${Math.round(v/totalVerdicts*100)}%)`,n]}/></PieChart>
                </ResponsiveContainer>
              </div>
              <div className="flex-1 space-y-1">
                {verdictData.slice(0,5).map(v=><div key={v.key} className="flex items-center gap-1.5"><span className="w-2 h-2 rounded-full flex-shrink-0" style={{backgroundColor:v.fill}}/><span className="text-[10px] text-gray-400 truncate flex-1">{v.name}</span><span className="text-[10px] font-bold font-[family-name:'JetBrains_Mono'] text-gray-300">{v.value}</span></div>)}
              </div>
            </div>
          ) : <div className="h-[120px] flex items-center justify-center text-gray-600 text-sm">No data</div>}
        </Card>
              ...
              {['codeforces','leetcode','codechef'].map(p => {
                ...
                return <div key={p}>
                  <div className="flex items-center justify-between mb-1">
                    <div className="flex items-center gap-1.5"><Ic className="w-3 h-3" style={{color:meta.color}}/><span className="text-[11px] font-medium text-gray-300">{meta.label}</span></div>
                    <span className="text-sm font-bold font-[family-name:'JetBrains_Mono'] text-white">{split.count} <span className="text-[10px] text-gray-500">({pct}%)</span></span>
                  </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-3">
          ...
          <Card delay={0.32}>
            ...
            <div className="space-y-2.5">
              {['codeforces','leetcode','codechef'].map(p => {
                ...
                return <div key={p} className="flex items-center gap-3 p-2 bg-[#0c0c0c] rounded-lg border border-[#1a1a1a]">
                  <div className="p-1.5 rounded-lg" style={{backgroundColor:meta.color+'15'}}><Ic className="w-4 h-4" style={{color:meta.color}}/></div>
                  <div className="flex-1"><p className="text-[9px] uppercase tracking-wider text-gray-500">{meta.label}</p><p className="text-lg font-bold font-[family-name:'JetBrains_Mono'] text-white">{cur}</p></div>
                  {d!==0&&<span className={`text-xs font-semibold ${d>0?'text-emerald-500':'text-red-500'}`}>{d>0?'↑':'↓'}{Math.abs(d)}</span>}
                </div>;
              })}
            </div>
          </Card>
          ...
          <Card delay={0.34}>
            ...
            <div className="space-y-2.5">
              {['codeforces','leetcode','codechef'].map(p => {
                ...
                return <div key={p} className="p-2 bg-[#0c0c0c] rounded-lg border border-[#1a1a1a]">
                  <div className="flex items-center justify-between mb-1.5">
                    <div className="flex items-center gap-1.5"><Ic className="w-3 h-3" style={{color:meta.color}}/><span className="text-[10px] text-gray-400">{meta.label}</span></div>
                    <div className="flex items-center gap-1 text-xs font-bold font-[family-name:'JetBrains_Mono']"><span className="text-white">{m.current}</span><ChevronRight className="w-3 h-3 text-gray-500"/><span style={{color:meta.color}}>{m.next}</span></div>
                  </div>
                  <div className="h-1.5 bg-[#1a1a1a] rounded-full overflow-hidden">
                  ...
                  <p className="text-[9px] text-gray-500 mt-1">{m.remaining} pts to go</p>
                </div>;
              })}
            </div>
          </Card>
        ...
        {paceProjection && paceProjection.projectedMonthly > 0 && (
          <Card delay={0.36}>
            ...
            <div className="space-y-2.5">
              {[{l:'Daily',v:paceProjection.dailyRate,u:'/day',c:'text-cyan-500'},{l:'Monthly',v:paceProjection.projectedMonthly,u:'/mo',c:'text-emerald-500'},{l:'Yearly',v:paceProjection.projectedYearly,u:'/yr',c:'text-violet-500'}].map(r=>(
                <div key={r.l} className="flex items-center justify-between p-2 bg-[#0c0c0c] rounded-lg border border-[#1a1a1a]">
                  <span className="text-[10px] uppercase tracking-wider text-gray-500">{r.l}</span>
                  <div><span className={`text-lg font-bold font-[family-name:'JetBrains_Mono'] ${r.c}`}>{r.v}</span><span className="text-[10px] text-gray-500 ml-1">{r.u}</span></div>
                </div>
              ))}
            </div>
          </Card>
        ...
        <Card delay={0.38}>
          ...
          {topTags?.length > 0 ? (
            <div className="flex flex-wrap gap-1.5">
              {topTags.map(t=><span key={t._id} className="inline-flex items-center gap-1 px-2 py-1 bg-emerald-500/10 border border-emerald-500/15 text-emerald-400 rounded-full text-[10px] font-semibold">{t._id}<span className="text-emerald-500/50 font-[family-name:'JetBrains_Mono']">{t.count}</span></span>)}
            </div>
          ...
          {languageDist?.length > 0 ? (
            <div className="space-y-2">
              {languageDist.slice(0,5).map((l,i)=><div key={l._id}>
                <div className="flex items-center justify-between mb-0.5"><span className="text-[11px] text-gray-300">{l._id||'Unknown'}</span><span className="text-[10px] font-bold font-[family-name:'JetBrains_Mono'] text-gray-400">{l.count}</span></div>
                <div className="h-1.5 bg-[#1a1a1a] rounded-full overflow-hidden"><motion.div initial={{width:0}} animate={{width:`${(l.count/maxLangCount)*100}%`}} transition={{delay:0.4+i*0.04,duration:0.5}} className="h-full bg-blue-500 rounded-full"/></div>
              </div>)}
            </div>
          ) : <p className="text-xs text-gray-600">No data</p>}
        </Card>
          ...
          <Card delay={0.42}>
            ...
            <div className="space-y-1.5 max-h-[200px] overflow-y-auto pr-1 custom-scrollbar">
              {recentContests.slice(0,6).map((c,i) => {
                ...
                return <div key={i} className="flex items-center gap-2 p-2 bg-[#0c0c0c] rounded-lg border border-[#1a1a1a] hover:border-[#333] transition-colors">
                  <div className="w-1 h-6 rounded-full flex-shrink-0" style={{backgroundColor:meta.color||'#888'}}/>
                  <div className="flex-1 min-w-0"><p className="text-[11px] font-medium text-gray-200 truncate">{c.name}</p><p className="text-[9px] text-gray-500">{ds}{c.rank?` · #${c.rank}`:''}</p></div>
                  <div className="text-right flex-shrink-0"><p className="text-xs font-bold font-[family-name:'JetBrains_Mono'] text-white">{c.rating}</p>
                    {c.delta!==0&&<p className={`text-[9px] font-semibold ${c.delta>0?'text-emerald-500':'text-red-500'}`}>{c.delta>0?'+':''}{c.delta}</p>}
                  </div>
                </div>;
              })}
            </div>
        ...
        <Card delay={0.44}>
          ...
          <ResponsiveContainer width="100%" height={140}>
            <BarChart data={difficultyDist.filter(d=>d._id&&d._id!=='0'&&d._id!=='').sort((a,b)=>(parseInt(a._id)||0)-(parseInt(b._id)||0))}>
              <CartesianGrid strokeDasharray="3 3" stroke="currentColor" opacity={0.06} vertical={false}/>
              <XAxis dataKey="_id" tick={{fill:'#666',fontSize:9}} axisLine={false} tickLine={false}/>
