...
export default function AddSnippetModal({ onClose, onAddLocal }) {
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 transition-opacity" onClick={onClose}>
            <div 
                className="bg-[#242424] border border-white/[0.12] rounded-2xl shadow-2xl w-full max-w-2xl max-h-[86vh] overflow-hidden flex flex-col animate-in fade-in zoom-in-95 duration-200" 
                onClick={e => e.stopPropagation()}
            >
                <div className="px-6 py-4 border-b border-white/[0.08] flex items-center justify-between bg-[#242424]">
                    <div className="flex items-center gap-3">
                        <div className="w-8 h-8 rounded-md bg-indigo-500/15 text-indigo-400 flex flex-shrink-0 items-center justify-center">
                            <Code2 size={16} />
                        </div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                            <div className="space-y-1.5 focus-within:text-indigo-600">
                                ...
                                <input
                                    ...
                                    required
                                    className="w-full px-4 py-2.5 text-sm bg-[#1A1A1A] border border-white/[0.12] rounded-lg text-gray-100 focus:outline-none focus:ring-2 focus:ring-white/30 focus:border-white/55 transition-colors placeholder:text-gray-500"
                                />
                            </div>
                            ...
                            <div className="space-y-1.5 focus-within:text-indigo-600">
                                ...
                                <div className="relative">
                                    <select 
                                        ...
                                        value={form.language} 
                                        onChange={handleChange}
                                        className="w-full px-4 py-2.5 text-sm bg-[#1A1A1A] border border-white/[0.12] rounded-lg text-gray-100 appearance-none focus:outline-none focus:ring-2 focus:ring-white/30 focus:border-white/55 transition-colors cursor-pointer"
                                    >
                ...
                <div className="px-6 py-4 border-t border-white/[0.08] bg-[#242424] flex justify-end gap-3 rounded-b-2xl">
                    <button 
                        type="button" 
                        className="px-5 py-2.5 text-sm font-medium text-gray-200 bg-[#1A1A1A] border border-white/[0.14] rounded-lg hover:bg-white/10 focus:outline-none transition-colors"
                        onClick={onClose}
                    >
                        Cancel
                    </button>
